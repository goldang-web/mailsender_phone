...
## 8. Sent 임계치 보호 및 IMAP 재확인
1. 대시보드에서 IMAP Sent 임계치를 1~2건으로 낮추고 테스트용 디바이스 두 대 이상을 등록한다.
2. SMTP 응답을 의도적으로 `452 4.7.1 Sent too many messages` 등 제한 코드가 반환되도록 유도하거나, 테스트 SMTP 스텁에서 동일한 문자열을 응답하도록 구성한다.
3. 클라이언트 로그에 `SMTP 제한 응답 감지` → `Sent 기준 확인용 테스트 메일 발송` → `IP 변경 성공/실패` → `IP 변경 후 IMAP 재확인` 메시지 순으로 출력되는지 확인한다.
4. Heartbeat 응답에서 각 디바이스의 IMAP 최근 상태가 "IP 변경 후 IMAP 재확인" 문구와 새 공인 IP를 포함하는지 확인하고, Sent 카운터가 성공 시 0·실패 시 임계치-1로 롤백되는지 점검한다.
5. 다중 디바이스 환경에서 한 단말의 IP 변경이 다른 단말의 Sent 카운터나 IMAP 플로우에 영향을 주지 않는지, QA 로그에 디바이스 ID별로 분리 기록되는지 확인한다.
